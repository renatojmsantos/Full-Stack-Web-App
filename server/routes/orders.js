...
router.get('/date/:creationdate', async(req, res) => {
    try{
...
router.post('/', async (req,res) =>{
    try{
        ...
        res.status(200).json("ORDER "+item_name+" successfully inserted!");
    }catch(err){
        res.status(500).json({message: err});
    }
})
...
router.patch('/quantity/:creationdate', async(req, res) => {
    try{
        const creationdate = req.params.creationdate;
        ...
        await db.query(query);
        res.status(200).json("QUANTITY of ORDER successfully updated");
    }catch(err){
        res.status(500).json({message: err});
    }
})
...
router.patch('/status/:creationdate', async(req, res) => {
    try{
        const creationdate = req.params.creationdate;
...
// ACCEPT ORDER 
// 1. Get info of order request on datetime
// 2. Check Stock of item for the quantity of order 
//      If there is no stock --> Alert User to fill stock! ; else continue to point 2
// 3. Status: complete (1)
// Create Stock Movement for item (e.g. quantity: -350)
// 4. Trace the list of stock to complete order (??) -> STOCK BEFORE -> STOCK AFTER ?
// 5. Send email to user
router.patch('/accept/:creationdate', async(req, res) => {
    try{
        const creationdate = req.params.creationdate;
        // 1. get order info
        ...
        if (stock_after < 0){
            res.status(400).json("NO STOCK TO ACCEPT! Please fill the stock with more quantity.");
        }else{
            ...
            try{
                ...
                res.status(200).json("ORDER ACCEPTED for item "+item_name+"! Stock before: "+current_stock+" After: "+stock_after);
            }catch(err){
